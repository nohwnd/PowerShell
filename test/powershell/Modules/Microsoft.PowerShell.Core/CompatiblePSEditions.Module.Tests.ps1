...
$global:_compat_oldModulePath = $env:PSModulePath

function global:Add-ModulePath
{
...
function global:Restore-ModulePath
{
    $env:PSModulePath = $global:_compat_oldModulePath
}

# Creates a new dummy module compatible with the given PSEditions
function global:New-EditionCompatibleModule
{
    param(
...
function global:New-TestModules
{
...
function global:New-TestNestedModule
{
    param(
        [string]$ModuleBase,
...
function global:Get-DesktopModuleToUse {
...
$global:desktopModuleToUse = Get-DesktopModuleToUse
...
Describe "Additional tests for Import-Module with WinCompat" -Tag "Feature" {
    ...
    Context "Tests around Windows PowerShell Compatibility NoClobber module list" {
        ...
        It "NoClobber WinCompat list in powershell.config is a Desktop-edition module" {
            ...
            if (-not $global:desktopModuleToUse) {
            ...
            $ConfigPath = Join-Path $TestDrive 'powershell.config.json'
@"
{"Microsoft.PowerShell:ExecutionPolicy": "RemoteSigned", "WindowsPowerShellCompatibilityNoClobberModuleList": ["$global:desktopModuleToUse"]}
"@ | Out-File -Force $ConfigPath
            ...
            $script = @"
Import-Module $global:desktopModuleToUse -UseWindowsPowerShell -WarningAction Ignore
Get-Module $global:desktopModuleToUse | ForEach-Object { `$_.ModuleType.ToString() }
(Get-PSSession | Measure-Object).Count
Remove-Module $global:desktopModuleToUse
(Get-PSSession | Measure-Object).Count
"@
...
'@
    ...
    It "Attempt to load a 'Desktop' edition module should fail because 'Export-PSSession' cannot be found" {
        ...
        $script = @"
    try {
        Import-Module $global:desktopModuleToUse -ErrorAction Stop
    } catch {
        `$_.FullyQualifiedErrorId
        `$_.Exception.Message
    }
"@
        ...
        $result[1] | Should -BeLike "*'$global:desktopModuleToUse'*'Export-PSSession'*'Microsoft.PowerShell.Utility'*"
    }
