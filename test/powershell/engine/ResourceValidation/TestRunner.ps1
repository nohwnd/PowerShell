...
function Test-ResourceStrings
{
    ...
    # Store in global scope so BeforeAll (which runs in an isolated scope) can access it
    $global:_TestResourceAssemblyName = $AssemblyName
    ...
    Describe "Resources strings in $AssemblyName (was -ResGen used with Start-PSBuild)" -Tag Feature {
        ...
        BeforeAll {
            ...
            $ASSEMBLY = [appdomain]::CurrentDomain.GetAssemblies()|
                Where-Object { $_.GetName().Name -eq $global:_TestResourceAssemblyName }
            $repoBase = (Resolve-Path (Join-Path $PSScriptRoot ../../../..)).Path
            $asmBase = Join-Path $repoBase "src/$global:_TestResourceAssemblyName"
            $resourceDir = Join-Path $asmBase resources
        ...
        AfterAll {
            Remove-Variable -Name '_TestResourceAssemblyName' -Scope Global -ErrorAction SilentlyContinue
        }
